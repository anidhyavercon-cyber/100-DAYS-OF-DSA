Problem1:Given intervals, merge all overlapping ones.
...
Problem2:Given an array of intervals where intervals[i] = [starti, endi], merge all overlapping intervals, and return an array of the non-overlapping intervals that cover all the intervals in the input.
Example 1:
...
Answer:
int** merge(int** intervals, int intervalsSize, int* returnSize, int** returnColumnSizes);
/* Use the same implementation as above since the problem is identical in logic */
